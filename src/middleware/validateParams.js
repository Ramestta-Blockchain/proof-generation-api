...
export default {
  ...
  validateV1NetworkParam: (req, res, next) => {
    ...
    try {
      // network can either be rama or pingaksha
      if (network !== 'rama' && network !== 'pingaksha') {
        return handleBadRequest({
          res,
          errMsg: `Invalid network ${network}. Network can either be rama or pingaksha for PoS v1 routes`
        })
      }
      next()
    } catch (error) {
